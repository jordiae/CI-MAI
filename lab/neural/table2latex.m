...
%                 if isinf(value), value = '$\infty$'; end
                if isstring(value)
                    temp{1,col} = convertStringsToChars(value);
                else
                    temp{1,col} = sprintf('%.6f',value);
                end
            end
%             if ~isempty(row_names)
...
%             end
        ...
        ME
%         error('2Unknown error. Make sure that table only contains chars, strings or numeric values.');
    end
